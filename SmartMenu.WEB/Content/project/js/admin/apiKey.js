...
function addUpdateAPIKey() {
    if ($("#frmAddUpdateAPIKey").valid()) {
        $.ajax({
            url: "/Admin/Configuration/AddUpdateStripeKey",
            method: "POST",
            data: $("#frmAddUpdateAPIKey").serialize(),
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (parseInt(result) === 1) {
                    toastr.success("API Key have been created successfully !", "Success", { timeOut: 3000 });
                }
                else if (parseInt(result) === 2) {
                    toastr.success("API Key have been updated successfully !", "Success", { timeOut: 3000 });
                }
                else {
...
function saveDeliveryAreaSetting() {
    if ($("#frmDeliverAreaSetting").valid()) {
        $.ajax({
            url: "/Admin/Configuration/AddUpdateDeliverAreaSetting",
            method: "POST",
            data: {
            ...
            success: function (result) {
                ajaxindicatorstop();
                if (parseInt(result) === 1) {
                    toastr.success("Setting have been updated successfully !", "Success", { timeOut: 3000 });
                }
                else {
...
function addUpdateCharges() {
    ...
    if ($("#frmAddUpdateCharges").valid()) {
        $.ajax({
            url: "/Admin/Configuration/AddUpdateTaxCharges",
            method: "POST",
            data: {

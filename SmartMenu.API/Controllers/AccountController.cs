...
using SmartMenu.API.Results;
using SmartMenu.DAL.Entity;
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetExternalLogin(string provider, string error = null)
        {
            ...
            if (hasRegistered)
            {
                ...
                AuthenticationProperties properties = ApplicationOAuthProvider.CreateProperties(user.UserName, user.Id, roleManager.FindById(user.Roles.FirstOrDefault().RoleId).Name);
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            if (!roleManager.RoleExists(RolesEnum.PluginAccess.ToString()))
            {
                roleManager.Create(new IdentityRole { Name = RolesEnum.PluginAccess.ToString() });
            }
            ...
            else if (model.RoleName.ToLower() == RolesEnum.PluginAccess.ToString().ToLower())
            {
                UserManager.AddToRole(user.Id, RolesEnum.PluginAccess.ToString());
            }
            #endregion

            #region "creating tedency for resturant"
            if (model.RoleName.ToLower() == EnumHelper.RolesEnum.Admin.ToString().ToLower())
            {
                ...
                string dbName = string.Format("{0}_{1}_{2}", model.Email.Substring(0, model.Email.IndexOf("@")).ToString(), DateTime.Now.Ticks, "SM");
        ...
        [Route("GetTenantId")]
        public string GetTenantId(string domainName)
        {
            string userId = string.Empty;
            using (DefaultConnection conn = new DefaultConnection())
            {
                userId = conn.TenantControl.Where(c => c.TenantDomain == domainName).Select(c => c.UserId).FirstOrDefault();
            }
            return userId;
        }
        #region Helpers
